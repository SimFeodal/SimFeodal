...
#for var in 0 2; do
    var2=$(($var + 1))
    echo taskset -c $var,$var2 bash gama-headless.sh -m 2048m -hpc 2 $experimentPath $outputPath$var
	taskset -c $var,$var2 bash gama-headless.sh -m 2048m -hpc 2 $experimentPath $outputPath$var &
done 
exit 0
